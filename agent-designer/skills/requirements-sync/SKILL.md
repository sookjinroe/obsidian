...
설계 확정(작업 G) 이후 플러그인 빌드 과정에서 발생하는 모든 요구사항 변경과 구현 완료를 추적하고, 설계 산출물을 살아있는 문서로 유지한다.
...
plugin-dev의 Phase 5(모든 SKILL.md 작성 완료)가 끝났을 때. artifact의 발화 목록을 skill-creator의 eval 발화 세트 형식으로 변환해 제공한다.
...
5. **재검증 범위 판단 및 재진입 준비**
   ...
   **핵심 처리 경로에 영향 → design-session 재진입 준비:**
   1. 변경 내용을 `artifact-final.md`에 반영한다
   2. 변경된 `artifact-final.md`를 `artifact-v4.md`로 복사한다 (gap-analyzer 입력 파일)
   3. `state.md`의 `current_step`을 `F`(구성요소 추가/제거 없는 경우) 또는 `D`(구성요소 구조 변경)로 설정한다
   4. 사용자에게 안내: "핵심 경로가 변경됐습니다. '설계 이어서 해줘'로 [작업 D/F]부터 재검증을 시작하세요."
   - design-session은 state.md를 읽어 설정된 단계(D 또는 F)부터 자동으로 재개한다

   **구현 완료된 구성요소에만 영향 → ②④⑤ 즉시 재검증 실행 (트리거 B 흐름으로 처리)**

   **경미한 변경 → Could 항목으로 artifact-final.md에 기록**
...
2. **② 입출력 불일치 검증**
   ...
   - artifact에서 이 구성요소 이후에 연결된 구성요소를 찾아 비교한다

3. **④ 도구 미연결 검증**
   - 이 Skill의 allowed-tools에 실제로 필요한 MCP 서버가 모두 있는가?
   - artifact의 계위 정보와 plugin-dev의 구현 명세를 비교한다

4. **⑤ 실패 경로 미처리 검증**
   - 이 구성요소가 실패할 때(API 오류, 데이터 없음, 타임아웃 등) 에이전트의 행동이 정의되어 있는가?
   - 정의되지 않은 실패 경로는 구멍으로 기록한다

5. **결과 기록**
   - 구멍 발견 시: `artifact-final.md`에 기록 + `state.md`의 `post_impl_gaps` 업데이트
   - 구멍 없음: `state.md`의 `implementation_status`에서 해당 구성요소를 `implemented`로 업데이트

6. **심각 구멍 처리**
   - ②④⑤에서 심각 구멍이 발견되면: plugin-dev에 수정 요청 안내
...
1. **artifact-final.md에서 발화-구성요소 연결 추출**
...
3. **eval 파일 생성**
   `.claude/agent-designer/eval-set.md`에 각 Skill별 eval 발화 목록을 작성한다:
