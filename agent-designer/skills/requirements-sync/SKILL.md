...
2. **영향 범위 역추적**
   - `{project_path}/artifact-final.md`를 읽는다
...
4. **산출물 업데이트**
   - `{project_path}/artifact-final.md`에 변경 사항 반영
   - 변경 이력 섹션에 기록:
     ```markdown
     ## 변경 이력
...
5. **재검증 범위 판단 및 재진입 준비**
   ...
   **핵심 처리 경로에 영향 → design-session 재진입 준비:**
   ...
   3. `state.md`의 `current_step`을 `F`(구성요소 추가/제거 없는 경우) 또는 `drafting`(구성요소 구조 변경)으로 설정한다
   4. 사용자에게 안내: "핵심 경로가 변경됐습니다. '설계 이어서 해줘'로 [구성요소 결정/gap 검증]부터 재검증을 시작하세요."
   - design-session은 state.md를 읽어 설정된 단계(drafting 또는 F)부터 자동으로 재개한다
...
5. **결과 기록**
   - gap 발견 시: `{project_path}/artifact-final.md`에 기록 + `{project_path}/state.md`의 `post_impl_gaps` 업데이트
