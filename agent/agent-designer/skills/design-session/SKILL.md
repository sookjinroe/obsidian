...
description: "Use this skill whenever the user wants to design, plan, or build a domain-specific agent or Claude Code plugin — even if they don't use the word 'design'. Trigger on: starting a new agent design (""에이전트 설계 시작"", ""플러그인 설계하고 싶어"", ""에이전트 만들려고 해"", ""자동화하고 싶어""), resuming an in-progress design (""설계 이어서 해줘"", ""plugin-dev:plugin-structure 완료됐어 이어서 진행해줘""), or any step within the design workflow (""페르소나 정의해보자"", ""발화 뽑아줘"", ""요구사항 추출해줘"", ""Walk-through 해보자"", ""gap 검증해줘"", ""설계 확정해줘"", ""커버리지 보완하자""). Do NOT trigger when: the user mentions requirement changes after finalization (""구현하다 보니 달라졌어"", ""plugin-dev가 구현 완료했어"" → use requirements-sync), or requests state file consistency check (""상태 동기화해줘"" → use state-reconcile). This skill guides the full agent design lifecycle from blank-slate context gathering through persona definition, utterance/requirement drafting, plugin-dev handoff, gap validation, and finalization."
version: 0.4.0
